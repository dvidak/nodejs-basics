...
describe('logged in', () => {
    ...
    it('it should update user', (done) => {
        request.put(`${endpoint}/20`,{json: true, body: {username: 'newUsername'}}, function (error, response) {
    ...
    it('it should not delete if user does not exkist', (done) => {
        request.delete(`${endpoint}/testUsernameDoesNotExist`, function (error, response) {
            expect(response.statusCode).toEqual(404);
            done();
        }).setHeader('x-access-token', token);
    });
